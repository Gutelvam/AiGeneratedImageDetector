...
from src.models import get_model # This import needs src.models to exist
from src.data_loader import get_dataloaders # This import needs src.data_loader to exist
import os
...
def train_model(model, train_loader, val_loader, criterion, optimizer, device, num_epochs=10, model_save_path="models/best_model.pth"):
    ...
    for epoch in range(num_epochs):
        ...
        # Validation Phase
        val_loss, val_accuracy, _, _, _, _, _, _ = evaluate_model(model, val_loader, criterion, device) # Adjusted to match evaluate_model return
...
def evaluate_model(model, data_loader, criterion, device):
    ...
    # Handle cases where precision/recall/f1 might not be defined for binary classification
    # (e.g., if only one class is present in the batch/data)
    precision, recall, f1, _ = precision_recall_fscore_support(all_labels, all_predictions, average='binary', zero_division=0)
    try:
        roc_auc = roc_auc_score(all_labels, all_probs)
    except ValueError:
        # This can happen if only one class is present in all_labels or all_probs.
        # In such cases, ROC AUC is not well-defined or meaningful.
        roc_auc = 0.5 # A default value for uncertainty or error
